...
class ModalID(object):
    ...
    def __init__(self, freq, frf):
        self.frf = frf
        self.freq = freq
        self.n_o = frf.shape[1]
        self.n_i = frf.shape[2]
        try:
    ...
    def p_lscf(
        ...
        exp_term_ = np.exp(
            ...
            * np.pi
            / d
            * np.einsum('ij,k->kij', r[:, np.newaxis], self.freq)
        )
        exp_term = exp_term_ @ np.conj(exp_term_).transpose(0, 2, 1)

        R = np.einsum("ijk->jk", exp_term).real
        S = (
            -np.einsum("ijk, ilm", self.frf, exp_term)
            .transpose(0, 2, 3, 1)
            .reshape(self.n_o, (max_order), (max_order) * self.n_i)
            .real
        )
        T = (
            np.einsum(
                "ijk, ilm",
                np.conj(self.frf).transpose(0, 2, 1) @ self.frf,
                exp_term,
            )
            .transpose(2, 0, 3, 1)
            .reshape((max_order) * self.n_i, (max_order) * self.n_i)
            .real
        )
        ...
        for n_p in n_p_all:
            ...
            if n_p != n_p_all[-1]:
                print(
                    f"Order:" + str(n_p + 1) + '/' + str(n_p_all[-1] + 1),
                    end="\r",
                )
            else:
                print(
            ...
            alpha = np.block(
                [
                    [
                        np.linalg.solve(
                            -M[: n_p * self.n_i, : n_p * self.n_i],
                            M[
                                : n_p * self.n_i,
                                n_p * self.n_i : (n_p + 1) * self.n_i,
                            ],
                        )
                    ],
                    [np.eye(self.n_i)],
            ...
            A = np.block(
                [
                    [
                        -alpha.reshape(n_p, self.n_i, self.n_i)[::-1]
                        .reshape(alpha.shape)
                        .T
                    ],
                    [np.eye(self.n_i * (n_p - 1), self.n_i * n_p)],
                ]
            )
            ...
            f_np, damp_np, p_np, L_np = self.transform_poles(
                eigval_t, eigvec, self.n_i, self.freq
            )
    ...
    def p_lsra(
        ...
        for n_p in n_p_all:
            if sol_type == 'linearized':
                ...
                rat = polyrat.StabilizedSKRationalApproximation(
                    n_p - 1, n_p, verbose=False
                )

                rat.fit(2 * np.pi * self.freq[:, None], self.frf)

                y = rat.denominator(2 * np.pi * self.freq[:, None])
                poly = polyrat.PolynomialApproximation(
                    n_p, Basis=polyrat.LegendrePolynomialBasis
                )
                poly.fit(2 * np.pi * self.freq[:, None], y)
                poles = 1.0j * poly.roots().ravel()
    ...
    def generate_p(
        self, frf_type, lower_residuals, upper_residuals, freq_rec=None
    ):
        """
        ...
        if m == 0:
            raise Exception(
                "No pole is selected. Select at least one pole on stability chart."
            )
        ...
        if frf_type == 'receptance':
            pO1r = -(
                (Li * si + Lr * sr - Li * w) / (sr**2 + (-si + w) ** 2)
            ) - (Lr * sr + Li * (si + w)) / (sr**2 + (si + w) ** 2)
            pO1i = (Li * sr + Lr * (-si + w)) / (sr**2 + (-si + w) ** 2) + (
                Li * sr - Lr * (si + w)
            ) / (sr**2 + (si + w) ** 2)

            pO2r = (Lr * si - Li * sr - Lr * w) / (
                sr**2 + (-si + w) ** 2
            ) + (Li * sr - Lr * (si + w)) / (sr**2 + (si + w) ** 2)
            pO2i = -(
                (Li * si + Lr * sr - Li * w) / (sr**2 + (-si + w) ** 2)
            ) + (Lr * sr + Li * (si + w)) / (sr**2 + (si + w) ** 2)
            ...
            if lower_residuals:
                pL1 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([1, 0])), -1 / w**2
                )
                pL2 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([0, 1])), -1 / w**2
                )

            if upper_residuals:
                pU1 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([1, 0])),
                    np.ones_like(w),
                )
                pU2 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([0, 1])),
                    np.ones_like(w),
                )

        elif frf_type == 'mobility':
            pO1r = (Lr * si * w - Li * sr * w + Lr * w**2) / (
                sr**2 + (si + w) ** 2
            ) + (Li * sr * w + Lr * (-(si * w) + w**2)) / (
                sr**2 + (-si + w) ** 2
            )
            pO1i = (Li * si * w + Lr * sr * w - Li * w**2) / (
                sr**2 + (-si + w) ** 2
            ) - (Li * si * w + Lr * sr * w + Li * w**2) / (
                sr**2 + (si + w) ** 2
            )

            pO2r = (-((Li * si + Lr * sr) * w) + Li * w**2) / (
                sr**2 + (-si + w) ** 2
            ) - (Li * si * w + Lr * sr * w + Li * w**2) / (
                sr**2 + (si + w) ** 2
            )
            pO2i = (Li * sr * w + Lr * (-(si * w) + w**2)) / (
                sr**2 + (-si + w) ** 2
            ) + (Li * sr * w - Lr * (si * w + w**2)) / (
                sr**2 + (si + w) ** 2
            )
            ...
            if lower_residuals:
                pL1 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([0, 1])), 1 / w
                )
                pL2 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([1, 0])), -1 / w
                )

            if upper_residuals:
                pU1 = np.kron(np.kron(np.eye(self.n_i), np.array([0, 1])), -w)
                pU2 = np.kron(np.kron(np.eye(self.n_i), np.array([1, 0])), w)

        elif frf_type == 'accelerance':
            pO1r = (Li * si * w**2 + Lr * sr * w**2 - Li * w**3) / (
                sr**2 + (-si + w) ** 2
            ) + (Li * si * w**2 + Lr * sr * w**2 + Li * w**3) / (
                sr**2 + (si + w) ** 2
            )
            pO1i = -(
                (-(Lr * si * w**2) + Li * sr * w**2 + Lr * w**3)
                / (sr**2 + (-si + w) ** 2)
            ) + (Lr * si * w**2 - Li * sr * w**2 + Lr * w**3) / (
                sr**2 + (si + w) ** 2
            )
            pO2r = (
                Lr * si * w**2 * ((-si + w) ** 2 - (si + w) ** 2)
                + Li * sr * w**2 * (-((-si + w) ** 2) + (si + w) ** 2)
                + Lr * w**3 * (2 * sr**2 + (-si + w) ** 2 + (si + w) ** 2)
            ) / ((sr**2 + (-si + w) ** 2) * (sr**2 + (si + w) ** 2))
            pO2i = (Li * si * w**2 + Lr * sr * w**2 - Li * w**3) / (
                sr**2 + (-si + w) ** 2
            ) - (Li * si * w**2 + Lr * sr * w**2 + Li * w**3) / (
                sr**2 + (si + w) ** 2
            )
            ...
            if upper_residuals:
                pU1 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([1, 0])), -(w**2)
                )
                pU2 = np.kron(
                    np.kron(np.eye(self.n_i), np.array([0, 1])), -(w**2)
                )

        else:
            raise Exception(
                'Unknown parameter frf_type; only "receptance", "mobility" or "accelerance" is allowed.'
            )
    ...
    def generate_p_proportional(
        ...
        if frf_type == 'receptance':
            ...
            pO1r = (Lr * (w**4 - w**2 * wr**2)) / (
                (-(w**2) + wr**2) ** 2 + 4 * w**2 * wr**2 * xir**2
            )

            pO2r = (2 * Lr * w**3 * wr * xir) / (
                (-(w**2) + wr**2) ** 2 + 4 * w**2 * wr**2 * xir**2
            )
            ...
            raise Exception(
                'Unknown parameter frf_type, please use receptance, mobility or accelerance.'
            )
    ...
    def p_lsfd(
        self,
        frf_type='receptance',
        assume_proportional=False,
        reconstruction=True,
        lower_residuals=True,
        upper_residuals=True,
        W=None,
        freq_rec=None,
        parsing=[None, None],
    ):
        ...
        if (np.array(parsing) == None).all():
            ...
            if not assume_proportional:
                ...
                m, P_ = self.generate_p(
                    frf_type, lower_residuals, upper_residuals, freq_rec=None
                )
            else:
                # proportional visous damping model
                m, P_ = self.generate_p_proportional(
                    frf_type, lower_residuals, upper_residuals, freq_rec=None
                )
            ...
            if not interval_range_2 > interval_range_1:
                raise Exception(
                    "Interval range 2 should be greater than interval range 1."
                )

            nat_freq_ind = np.array(
                [
                    np.where(
                        np.isclose(
                            self.freq, _, atol=self.freq[1] - self.freq[0]
                        )
                    )[0][0]
                    for _ in self.nat_freq
                ]
            )

            level_1_intervals = nat_freq_ind[:, None] + np.array(
                [-interval_range_1, interval_range_1]
            )
            level_1_ind = np.array(
                [np.arange(*_) for _ in level_1_intervals]
            ).ravel()
            level_1_ind = np.unique(level_1_ind[level_1_ind > 0])

            level_2_intervals = nat_freq_ind[:, None] + np.array(
                [-interval_range_2, interval_range_2]
            )
            level_2_ind = np.array(
                [np.arange(*_) for _ in level_2_intervals]
            ).ravel()
            ...
            Y_ = np.block(
                [[[self.frf.real[ind_list]]], [[self.frf.imag[ind_list]]]]
            )
        ...
        if W == None:
            O_ = np.linalg.lstsq(
                (P_.transpose(1, 0, 2).reshape(-1, P_.shape[-1])),
            ...
            if W.shape == self.freq.shape:
                W_ = np.diag(np.tile(np.tile(W, self.n_i), 2))
                O_ = np.linalg.lstsq(
                    W_**0.5
                    @ (P_.transpose(1, 0, 2).reshape(-1, P_.shape[-1])),
                    W_**0.5
                ...
                raise Exception(
                    'Invalid W; shapes of W and freq should be the same.'
                )
        ...
        if not self.assuming_proportional:
            ...
            O = (O_[:m] + 1.0j * O_[m : 2 * m]).T
            self.r = np.einsum("om, im -> moi", O, self.selected_mpf)
            self.shape = O

            # residual residues - complex
            if lower_residuals and upper_residuals:
                RL_, RU_ = np.split(O_[2 * m :], [-2 * self.n_i])

                self.rl = (RL_[::2] + 1.0j * RL_[1::2]).T
                self.ru = (RU_[::2] + 1.0j * RU_[1::2]).T
                ...
                self.rl = np.zeros((self.n_o, self.n_i))
                self.ru = np.zeros((self.n_o, self.n_i))
            ...
            if lower_residuals and upper_residuals:
                AL_, AU_ = np.split(O_[m:], [-2 * self.n_i])

                self.al = (AL_[::2] + 1.0j * AL_[1::2]).T
                self.au = (AU_[::2] + 1.0j * AU_[1::2]).T
                ...
                self.al = np.zeros((self.n_o, self.n_i))
                self.au = np.zeros((self.n_o, self.n_i))
        ...
        if reconstruction:
            if freq_rec != None:
                if not self.assuming_proportional:
                    ...
                    # proportional visous damping model
                    _, P_ = self.generate_p_proportional(
                        frf_type,
                        lower_residuals,
                        upper_residuals,
                        freq_rec=freq_rec,
            ...
            if not (
                np.array(parsing) == None
            ).all():  # if parsing is not None, than P_ needs to be reevaluated before reconstruction due to previous slicing
                if not self.assuming_proportional:
                    # general viscous damping model
                    _, P_ = self.generate_p(
                        frf_type,
        ...
        if not self.assuming_proportional:
            ...
            for r, R_r in enumerate(self.r):
                ...
                if check_dp:
                    ...
                    ind_2 = np.where(
                        np.abs(dp_values) < 0.01 * np.mean(np.abs(dp_values))
                    )[0]
                    ...
                    if len(dp_values) > 0:
                        ...
                        print(
                            ...
                            + str(ndp)
                            + '/'
                            + str(len(output_dp_ind))
                        )
                    else:
                        raise Exception(
                            'For at least one mode none of the provided driving point values seems to be physically valid. If you want to proceed anyway, apply check_dp = False.'
                        )
                ...
                b_output = np.hstack(
                    R_r[:, input_dp_ind_].T
                )  # stacked residue columns at input_dp_dof ind
                a_output = (
                    np.repeat(dp_values**0.5, self.r.shape[1])[:, None]
                    * np.tile(np.eye(self.r.shape[1]), ndp).T
                )
                x_abs_output = np.linalg.lstsq(a_output, np.abs(b_output))[0]
                psi_o.append(x_abs_output * np.exp(1.0j * np.angle(R_r[:, 0])))

                # rows -> modal participation factors
                b_input = np.hstack(
                    R_r[output_dp_ind_, :]
                )  # stacked residue rows at output_dp_dof ind
                a_input = (
                    np.repeat(dp_values**0.5, self.r.shape[2])[:, None]
                    * np.tile(np.eye(self.r.shape[2]), ndp).T
                )
                x_abs_input = np.linalg.lstsq(a_input, np.abs(b_input))[0]
                psi_i.append(x_abs_input * np.exp(1.0j * np.angle(R_r[0, :])))
            ...
            for r, A_r in enumerate(self.a):
                ...
                if check_dp:
                    ...
                    if len(dp_values) > 0:
                        ...
                        raise Exception(
                            'For at least one mode none of the provided driving point values seems to be physically valid. If you want to proceed anyway, apply check_dp_values = False.'
                        )
                ...
                a_output = (
                    np.repeat(dp_values**0.5, self.a.shape[1])[:, None]
                    * np.tile(np.eye(self.a.shape[1]), ndp).T
                )
                x_abs_output = np.linalg.lstsq(a_output, np.abs(b_output))[0]
                ...
                a_input = (
                    np.repeat(dp_values**0.5, self.a.shape[2])[:, None]
                    * np.tile(np.eye(self.a.shape[2]), ndp).T
                )
                x_abs_input = np.linalg.lstsq(a_input, np.abs(b_input))[0]
    ...
    @staticmethod
    def transform_poles(poles, mpf, n_i, freq):
        """
        ...
        ind = np.array([f > np.min(freq), f < np.max(freq), damp > 0]).all(
            axis=0
        )
        f_pos = f[ind]
    ...
    def complex_to_closest_real(c_all):
        ...
        for c in c_all.T:
            ...
            result = least_squares(
                func, x0, args=(cr, ci), bounds=(lower_bounds, upper_bounds)
            )
            r_all.append(result.x[:-1])
        return np.array(r_all).T

    @staticmethod
    def select_stable_poles(
        stab_plot, L, order, freq_n1, damp_n1, stab_f, stab_damp, stab_mpf
    ):
        """
        ...
        # prepare datasets
        poles_n = stab_plot[
            np.where(stab_plot[:, 0] == np.amax(stab_plot[:, 0]))
        ]
        freq_n = poles_n[:, 1]
        damp_n = poles_n[:, 2]
        ...
        _ind = (np.abs(freq_n1 - freq_n) / freq_n)[
            np.arange(freq_n.shape[0]), ind
        ] < stab_f
        ind_n1 = ind[_ind]
        ...
        _ind = (
            np.abs(damp_n1[ind_n1] - damp_n[ind_n]) / damp_n[ind_n] < stab_damp
        )
        ...
        ind_L = ind_damp_n1[
            1
            - np.diag(
                mac(L[-1][0][:, ind_damp_n1], L[-2][0][:, ind_damp_n]).real
            )
            < stab_mpf
        ]
        pole_type[ind_L] += 1
        ...
        new_stab_plot = np.array(
            [
                np.repeat(order + 1, freq_n1.shape[0]),
                freq_n1,
                damp_n1,
                pole_type,
            ]
    ...
    def p_l_from_index(self, index):
        ...
        p_L_i = np.argwhere(
            np.unique(self.stab_plot[:, 0]) == i_order
        ).ravel()[0]
