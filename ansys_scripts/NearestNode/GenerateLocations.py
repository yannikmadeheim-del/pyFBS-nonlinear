...
for impact in impact_rp:
    all = [impact.CoordinateSystem.XAxis,impact.CoordinateSystem.YAxis,impact.CoordinateSystem.ZAxis]

    for one in all:
        _X = str(impact.XCoordinate).split(" ")
        _Y = str(impact.YCoordinate).split(" ")
        _Z = str(impact.ZCoordinate).split(" ")

        _X1 = str(impact.CoordinateSystem.OriginX).split(" ")
        _Y1 = str(impact.CoordinateSystem.OriginY).split(" ")
        _Z1 = str(impact.CoordinateSystem.OriginZ).split(" ")

        impact_data.append([impact.CoordinateSystem.Name,float(_X1[0])+float(_X[0]),_X[1],float(_Y1[0])+float(_Y[0]),_Y[1],float(_Z1[0])+float(_Z[0]),_Z[1],one[0],one[1],one[2]])
...
for sensor in sensort_rp:
    all = [sensor.CoordinateSystem.XAxis,sensor.CoordinateSystem.YAxis,sensor.CoordinateSystem.ZAxis]

    for one in all:
        _X = str(sensor.XCoordinate).split(" ")
        _Y = str(sensor.YCoordinate).split(" ")
        _Z = str(sensor.ZCoordinate).split(" ")

        _X1 = str(sensor.CoordinateSystem.OriginX).split(" ")
        _Y1 = str(sensor.CoordinateSystem.OriginY).split(" ")
        _Z1 = str(sensor.CoordinateSystem.OriginZ).split(" ")

        sensor_data.append([sensor.CoordinateSystem.Name,float(_X1[0])+float(_X[0]),_X[1],float(_Y1[0])+float(_Y[0]),_Y[1],float(_Z1[0])+float(_Z[0]),_Z[1],one[0],one[1],one[2]])
