...
def plot(var_item, off_screen=False, full_screen=False, screenshot=None,
         ...
         show_bounds=True, show_axes=True, notebook=False, background=None,
         text='', **kwargs):
    """
...
class Plotter(object):
    ...
    def __init__(self, off_screen=False, notebook=False):
        ...
        self.selected_nodes_hammer = list()
        self.observer_h = None
        self.observer_d = None
        self.selected_nodes_sensor = list()
    ...
    def key_press_event(self, obj, event):
        ...
        if key == 'q':
            ...
            self.iren.RemoveObserver(self.observer_d)
            self.observer_h = self.iren.AddObserver('LeftButtonPressEvent',
                                                      self.Hammer)
            
        
        elif key == "d":
            self.iren.RemoveObserver(self.observer_h)
            self.observer_d = self.iren.AddObserver('LeftButtonPressEvent',
                                                      self.Sensor)

    def Hammer(self, obj, eventType):
        ...
        if(actor.GetProperty().GetRepresentation() == 0):
            actor.GetProperty().SetColor(255, 0, 0)
            actor.GetProperty().SetDiffuse(1.0)
            actor.GetProperty().SetSpecular(0.0)
            Temp = actor.GetCenter()
            Temp = str(Temp)
            Temp = Temp.split(",")
            a = Temp[0].replace("(","")
            b = Temp[1].replace(" ","")
            ...
            abc = [a,b,c]
            Temp = ",".join(abc)
            self.selected_nodes_hammer.append(Temp)
        
    def Sensor(self, obj, eventType):
        # Get 2D click location on window
        clickPos = self.iren.GetEventPosition()
        ...
        if(actor.GetProperty().GetRepresentation() == 0):
            ...
            abc = [a,b,c]
            Temp = ",".join(abc)
            self.selected_nodes_sensor.append(Temp)
    ...
    def add_mesh(self, mesh, color=None, style=None,
        ...
        if isinstance(mesh, np.ndarray):
            ...
            for i in range (0,mesh.shape[1]):    
                temp_mesh = vtki.PolyData(mesh[:,i,:])
                # set main values
                self.mesh = temp_mesh
                self.mapper = vtk.vtkDataSetMapper()
                self.mapper.SetInputData(self.mesh)
   
                temp_actor, temp_prop = self.add_actor(self.mapper)
        ...
        elif style == 'points' and isinstance(mesh, np.ndarray):
            for i in range (0,mesh.shape[1]):    
                prop[i].SetRepresentationToPoints()
        elif style == "points" and isinstance(mesh, np.ndarray) == False:
        ...
        if isinstance(mesh, np.ndarray):
            for i in range (0,mesh.shape[1]):
                prop[i].SetColor(rgb_color)
                prop[i].SetOpacity(opacity)
        else:
