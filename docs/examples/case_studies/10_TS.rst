...
*****************************
As already shown in the `3D Display <https://pyfbs.readthedocs.io/en/master/examples/basic_examples/01_static_display.html>`_ one can load the predefined datasets from an example and add a structure from STL file to the 3D display. This allows both the acceleration sensors and excitation points to be visualized. Also for the transmission simulator application, a subplot representation, as already presented in `Coupling <https://pyfbs.readthedocs.io/en/master/examples/fbs/07_coupling.html>`_, can be used.
...
****************************
The VPT can be performed on experimental data. See the :download:`05_VPT.ipynb <../../../examples/05_VPT.ipynb>` example for more options and details.
...
.. code-block:: python
	...
	vpt_TS = pyfbs.interface.VPT(df_chn_TS,df_imp_TS,df_vp,df_vpref,sort_grouping = False)
	vpt_BTS = pyfbs.interface.VPT(df_chn_BTS,df_imp_BTS,df_vp,df_vpref,sort_grouping = False)
	vpt_A = pyfbs.interface.VPT(df_chn_A,df_imp_A,df_vp,df_vpref,sort_grouping = False)

	vpt_TS.apply_vpt(freq,Y_TS)
	vpt_BTS.apply_vpt(freq,Y_BTS)
	vpt_A.apply_vpt(freq,Y_A)

	Y_TS_tran = vpt_TS.frf
	Y_BTS_tran = vpt_BTS.frf
	Y_A_tran = vpt_A.frf
...
.. code-block:: python
	...
	Y_int = Bu@Y_AnB@Bf.T
	Y_ABn = Y_AnB - Y_AnB@Bf.T@np.linalg.pinv(pyfbs.tsvd(Y_int,trunc = 22))@Bu@Y_AnB
