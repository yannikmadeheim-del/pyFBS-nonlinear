...
def m_semm(
    ...
    """
    This function performs M-SEMM, System Equivalent Model Mixing in modal
    domain. Hybrid model is generated from a numerical and an experimental
    models using a substructuring approach.
    ...
    :param semm_type: Selection of the applied decoupling constraints
        (compatibility and equilibrium).
    :type semm_type: str('basic', 'extended')
    :param tsvd_rcond: Relative condition number for small singular value
        trucation tp weaken the extended decoupling constraints.
    :type tsvd_rcond: float
    :param ns_rcond: Relative condition number for the null-space hybrid
        eigenvector evaluation.
    :type ns_rcond: float

    :return: hybrid modal parameters
        (eigenvalues, damping ratios, eigenvectors)
    :rtype: array(float)
    """
    ...
    if (
        not eig_val_num.shape[0]
        == damping_num.shape[0]
        == eig_vec_num.shape[1]
    ):
        raise Exception(
            'Numerical model - Number of modes at the given modal parameters '
            'does not match.'
        )
    if (
        not eig_val_exp.shape[0]
        == damping_exp.shape[0]
        == eig_vec_exp.shape[1]
    ):
        raise Exception(
    ...
    eig_vec_par = np.copy(eig_vec_num)[
        np.hstack([i_dof_ind_num, b_dof_ind_num]), :
    ]
    ...
    if n_b >= m_par:
        ...
        B_c = np.block(
            [np.zeros([n_b, m_par]), eig_vec_par[-n_b:], -eig_vec_ov]
        )
            ...
            if semm_type == 'basic':
                ...
                if np.isclose(denom, 0).any():
                    ...
                    inp = np.einsum(
                        'kij,k->ij',
                        np.einsum(
                            "ik,jk->kij", eig_vec_par, eig_vec_par[-n_b:]
                        ),
                        1 / denom,
                    )
        ...
        def generate_Dsemm(Dov, Dpar, N3):
            return np.block(
                [
                    [Dov, np.zeros([m_ov, n_s]), P_12.T @ Dpar @ N3],
                    [
                        np.zeros([n_s, m_ov]),
                        np.diag(np.random.random(n_s) * 1e-20),
                        N_1.T @ Dpar @ N3,
                    ],
        ...
        for k, (lam_k, phi_ov_k) in enumerate(zip(_eig_val_ov, _eig_vec_ov.T)):
            # One evaluation for 'basic' and "frequency" dependent evaluations
            # for 'extended' formulation
            ...
            try:
                _phi_k = sp.linalg.null_space(
                    K_semm - lam_k * M_semm, rcond=ns_rcond
                )
                np.argmax(_phi_k)
            except:
                # Just in case the null-space evaluation fails
                print('Eigenvalue solver employed.')
                lam_k, _phi_k = sp.sparse.linalg.eigs(
                    K_semm, k=1, M=M_semm, sigma=lam_k
                )
            ...
            if phi_k.shape[1] > 1:
                ...
                true_ind = [
                    np.argmax(
                        mac(
                            phi_k[-n_b:], phi_ov_k, output_type='diagonal'
                        ).real
                    )
                ]
