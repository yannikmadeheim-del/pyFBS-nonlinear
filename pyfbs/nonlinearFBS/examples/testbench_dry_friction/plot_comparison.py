"""
Overlay of dry-friction testbench forced-response references straight from
...
columns with u(t) = Re( sum_h a_h exp(1j h omega t) ), as written by the
pyFBS nonlinearFBS examples and any pyhbm reference following the same
convention.
...
HARMONICS = np.arange(1,20,2)        # exported harmonics (identical in all CSVs)
N_T       = 1024                # common dense time grid for the period maxima
OUT       = "A_S1X"             # plotted output channel (uout of every export)
TRANS     = ("ux", "uy", "uz")  # translational gap DoFs (friction x-y, spring z)
XLIM      = (15, 5000)            # displayed frequency range [Hz]
...
overlay_figure(curves, labels, "out_max", "out_h1",
               f"Dry-friction testbench A + B -- output channel {OUT}",
               "max_t |u_out(t)|  [m]", f"|a_1({OUT})|  [m]")
overlay_figure(curves, labels, "gap_max", "gap_h1",
               "Dry-friction testbench A + B -- relative VP displacement"
