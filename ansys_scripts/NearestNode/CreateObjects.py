...
for CSYS in ExtAPI.DataModel.Project.Model.CoordinateSystems.Children:
    if "H" in CSYS.Name:
        ...
        if "Coordinate System" in CSYS.Name:
            pass
...
# Create Coresponding NamedSelections
def generate_node_NS(CSYS,name):
    allfaces = ExtAPI.DataModel.Project.Model.AddNamedSelection()
    allfaces.Name = name
    selws = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.WorksheetSpecific)
    allfaces.Location = selws
    allfaces.ToleranceType = allfaces.ToleranceType.Manual

    allfaces.RelativeTolerance = 1e-20
    allfaces.ZeroTolerance = 1e-20

    faces = allfaces.Location

    faces.AddRow()
    faces.SetEntityType(0,NamedSelectionWorksheetEntityType.MeshNode)
    faces.SetCriterion(0,NamedSelectionWorksheetCriterion.NodeId)
    faces.SetOperator(0,NamedSelectionWorksheetOperator.GreaterThan)
    faces.SetValue(0,-1)

    faces.AddRow()
    faces.SetAction(1,NamedSelectionWorksheetAction.Filter)
    faces.SetEntityType(1,NamedSelectionWorksheetEntityType.MeshNode)
    faces.SetCriterion(1,NamedSelectionWorksheetCriterion.Distance)
    faces.SetCoordinateSystemSelection(1,CSYS.InternalObject.ID)
    ...
    #for i in range(NoExtend):
    #    faces.SetOperator(1,NamedSelectionWorksheetOperator.LessThan)
    #    faces.SetValue(1,(i+1)*Interval)
    #    faces.Generate()
    #    if allfaces.TotalSelection > NoNodes:
    #        break
    #faces.Generate()

    #Smallest
    faces.SetOperator(1,NamedSelectionWorksheetOperator.Smallest)
    faces.Generate()
    return allfaces

# Create Named selections
impact_NS = []
for i,csys in enumerate(impact_csys):
    NS = generate_node_NS(csys, "H"+str(i+1))
    impact_NS.append(NS)

sensor_NS = []
for i,csys in enumerate(sensor_csys):
    NS = generate_node_NS(csys, "S"+str(i+1))
    sensor_NS.append(NS)
...
for _rp,impact in enumerate(impact_rp):
    ...
    # Set selection to named selection
    impact.Location = impact_NS[_rp]
