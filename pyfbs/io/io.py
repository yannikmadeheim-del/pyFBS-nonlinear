...
LAB_FILES = {
    ...
    "Measurements": [
        "AM_Measurements.xlsx",
        "ammeasurements.xlsx",
        "coupling_example.xlsx",
        "decoupling_example.xlsx",
        "decoupling_example_SVT.xlsx",
        "TPA_synt.xlsx",
        "Y_A.p",
        "Y_B.p",
        "Y_AB.p",
        "expansion_synt_example.xlsx",
    ],
}

AUTOMOTIVE_FOLDER = "automotive_testbench"
AUTOMOTIVE_FILES = {
    "FEM": ["EM.full", "EM.rst", "RM.full", "RM.rst", "TM.full", "TM.rst"],
    "STL": [
        "engine_mount.stl",
        "receiver.stl",
        "roll_mount.stl",
    ...
    "Measurements": [
        ...
        "TS.xlsx",
        "TS.xlsx",
        "frame_rubbermounts.p",
        "frame_rubbermounts_sourceplate.p",
        "modal.xlsx",
    ],
}
...
def load_uff_file_pak(
    uff_file_data, uff_file_output, uff_file_input, chn_input=False
):
    """
    Loads an Universal File Format .uff file from PAK system and parses the
    data in arrays and DataFrames

    :param uff_file_data: A filename of .uff file containing information on
        FRFs
    :type uff_file_data: str
    :param uff_file_output: A filename of .uff file containing information on
        channels
    :type uff_file_output: str
    :param uff_file_input: A filename of .uff file containing information on
        reference channels
    :type uff_file_input: str
    :returns: frequency vector, FRF matrix, channel DataFrame,
        impact DataFrame, sensor DataFrame
    """
    ...
    for _out in range(chn_dof):
        for _in in range(imp_dof):
            ...
            Name = (
                'S'
                + str(math.ceil(Node_Number / _value))
                + " "
                + Directions[data[i]['rsp_dir']]
            )
            ...
            RefName = (
                'H' + str(Node_Number) + " " + Directions[data[i]['ref_dir']]
            )
...
def download_automotive_testbench(overwrite=False):
    ...
    for sub_dir in url_a_sub:
        ...
        for filename in tqdm(url_a_sub[sub_dir]):
            ...
            if not (
                os.path.isfile(
                    folder_name + os.sep + sub_dir + os.sep + '%s' % filename
                )
            ):
                # download each file
...
def download_lab_testbench(overwrite=False):
    ...
    url_lab = (
        r'https://gitlab.com/pyFBS/pyFBS_data/-/raw/master/lab_testbench/'
    )
...
def load_hdf_frfs(file_name):
    """
    Loads HDF file format - Math (FRFs from Modal test), when exported from
    Dewesoft 2021.4 software.

    :param uff_file_data: A filename of the .hdf file containing information
        of FRFs
    :type uff_file_data: str
    """
...
def load_hdf_ai(file_name, group_name="AI"):
    ...
    :param uff_file_data: A filename of the .hdf file containing information of
        FRFs
    :type uff_file_data: str
    """
