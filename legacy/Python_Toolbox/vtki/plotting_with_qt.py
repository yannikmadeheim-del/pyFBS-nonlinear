...
from PyQt5.QtWidgets import QApplication, QMainWindow, QFrame, QVBoxLayout, QMenu
...
class MainWindow(QMainWindow):
    ...
    def contextMenuEvent(self,event):
        ...
        hammer_act = menu.addAction("Hammer Point")
        sensor_act = menu.addAction("Sensor Point")

        action = menu.exec_(self.mapToGlobal(event.pos()))
        
        if action == hammer_act:
            self.a = "h"
        elif action == sensor_act:
            self.a = "s"
...
class Plotter(object):
    ...
    def __init__(self, window, off_screen=False, notebook=False):
        """
        ...
        self.window.widget.GetRenderWindow().AddRenderer(self.renderer)

        if self.off_screen:
            self.ren_win.SetOffScreenRendering(1)
        else:  # Allow user to interact
            self.iren = self.window.widget
